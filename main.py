...
def BMI():
    ...
    if (int(bmi) >= 0 and int(bmi) <= 18.5):
        ...
        label2.config(text="Super overweight!!!")
    else:
...
#command to chance bg color of scale
stye2 = ttk.Style()
stye2.configure("TScale",backgroud="white")
slider2=ttk.Scale(root,from_=0,to=200,orient='horizontal',style="TScale",
                 command=slider_changed2,variable=current_value2)
slider2.place(x=300,y=237)
##########End_slider_weight##########

#Entry box 
Height=StringVar()
Weight=StringVar()
height=Entry(root,textvariable=Height,width=5,font="arial 45",bg= "#fff",fg="#000",bd=0,justify=CENTER)
height.place(x=35,y=140)
Height.set(get_current_value())

weight=Entry(root,textvariable=Weight,width=5,font="arial 45",bg= "#fff",fg="#000",bd=0,justify=CENTER)
weight.place(x=255,y=140)
Weight.set(get_current_value2())
...
Button(root,text="View Report",width=15,height=2,font="arial 10 bold",bg="#1f6e68",fg="white",command=BMI).place(x=280,y=305)
...
label3=Label(root,font="arial 60 bold",bg="lightblue",fg="#fff")
label3.place(x=125,y=305)
